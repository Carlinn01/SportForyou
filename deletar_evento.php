...
if ($evento['organizador_id'] != $idusuario_logado) {
    header("Location: eventos.php?erro=" . urlencode("Você não tem permissão para deletar este evento"));
    exit;
}
...
if (EventoDAO::deletar($idevento, $idusuario_logado)) {
    header("Location: eventos.php?mensagem=" . urlencode("Evento deletado com sucesso!"));
} else {
    header("Location: eventos.php?erro=" . urlencode("Erro ao deletar evento"));
}
exit;
