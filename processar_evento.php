...
if ($idevento && is_numeric($idevento)) {
    ...
    if (!$evento || $evento['organizador_id'] != $idusuario_logado) {
        header("Location: eventos.php?erro=" . urlencode("Evento não encontrado ou sem permissão"));
        exit;
    }
    ...
    if (EventoDAO::atualizar($idevento, $dados)) {
        header("Location: eventos.php?mensagem=" . urlencode("Evento atualizado com sucesso!"));
    } else {
    ...
    if ($novoEventoId) {
        header("Location: eventos.php?mensagem=" . urlencode("Evento cadastrado com sucesso!"));
    } else {
